 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
   ...
   // The cost is stored is the first element of `fg`.
    ...
    //from post https://discussions.udacity.com/t/mpc-cost-paramter-tuning-question/354670
    //add the control over delta*v
    for (int t = 0; t < N - 1; t++) {
  ...
  for (int t = 1; t < N ; t++) {
    ...
    //note: we use 3 order so not same as the lecture code.
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs,
  ...
  //!! initial state should not be with bound as 0
  ...
  y_list->clear();
  for (int i = 0; i < N-1; i++) {
    x_list->push_back(solution.x[x_start + i + 1]);
    y_list->push_back(solution.x[y_start + i + 1]);
  }
